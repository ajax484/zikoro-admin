...
export const useGetAdminEvents = ({
  ...
  const getEvents = async () => {
   setLoading(true);
    ...
    try {
      const { data, status } = await getRequest<TPaginatedResponse<Event>>({
      ...
      if (
        data.data === null ||
        (Array.isArray(data.data.responseData) && data.data.responseData.length === 0)
      )
        return setHasReachedLastPage(true);
      setEvents((prev) => _.uniqBy([...data.data.responseData], "id"));
...
export const useGetAdminEventTransactions = ({
  ...
  const getTransactions = async () => {
    ...
    try {
      const { data, status } = await getRequest<TPaginatedResponse<Transactions>>({
      ...
      if (
        ...
        (Array.isArray(data.data.responseData) &&
          data.data.responseData.length === 0)
      )
        return setHasReachedLastPage(true);
      setTransactions(_.uniqBy([...data.data.responseData], "id"));
      setTotal(data?.data?.totalPages);
    } catch (error) {
      setError(true);
    } finally {
