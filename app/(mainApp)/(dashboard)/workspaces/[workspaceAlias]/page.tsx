...
const OverviewTab = ({ workspace }: { workspace: any }) => {
  const info = [
    ...
    { label: "Default Currency", value: workspace.prefixPreferences?.sku?.prefix === "SKU" ? "USD" : "NGN", icon: CurrencyDollarIcon },
  ];
              ...
              {[
                ...
                <div key={i} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className={cn("text-sm font-medium", step.status ? "text-slate-900" : "text-slate-400")}>{step.label}</span>
                  </div>
                  {step.status ? (
  ...
  return (
    <Card className="border-none shadow-sm overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700">Invite Member</Button>
      </CardHeader>
...
export default function WorkspaceDetailsPage() {
  ...
  if (isFetching && !workspace) {
    return <div className="p-10 space-y-8"><Skeleton className="h-20 w-full" /><Skeleton className="h-96 w-full" /></div>;
  }
  ...
  const initials = workspace.organizationName?.split(" ").map((n: any) => n[0]).join("").toUpperCase().slice(0, 2);
    ...
    <div className="p-6 md:p-10 max-w-[1400px] mx-auto min-h-full space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" className="h-11 px-5 font-semibold text-slate-600 hover:text-indigo-600 transition-all border-slate-200">
            ...
            <Button className="h-11 px-6 font-bold bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-600/20">
              Launch Workspace <ArrowSquareOutIcon weight="bold" className="ml-2" />
            </Button>
          </div>
        </div>
      </div>
             ...
             <CardContent>
               <div className="flex flex-col md:flex-row gap-8 items-start">
                 <div className="flex-1 space-y-6 w-full">
                    <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between">
                      <div className="space-y-1">
                        ...
                        <h2 className="text-2xl font-black text-slate-900">{workspace.subscriptionPlan || "Basic"} Plan</h2>
                      </div>
                      <Badge className="bg-indigo-600 px-4 py-1.5 font-bold uppercase tracking-widest text-[10px]">Upgrade</Badge>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="p-4 border rounded-xl border-slate-100">
                        ...
                        <p className="text-sm font-bold text-slate-700 mt-1">
                          {workspace.subscriptionExpiryDate ? new Date(workspace.subscriptionExpiryDate).toLocaleDateString() : "N/A"}
                        </p>
                      </div>
                      <div className="p-4 border rounded-xl border-slate-100">
                 ...
                 <div className="w-full lg:w-80 space-y-4">
                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Workspace Quotas</h4>
                    {[
                      { label: "Team Members", current: workspace.usersCount, max: 5 },
                      { label: "Active Products", current: workspace.productsCount, max: 1000 },
                    ].map((quota, i) => (
                      <div key={i} className="space-y-2">
                        <div className="flex justify-between text-xs font-bold">
                          <span className="text-slate-500">{quota.label}</span>
                          <span className="text-slate-900">{quota.current} / {quota.max}</span>
                        </div>
